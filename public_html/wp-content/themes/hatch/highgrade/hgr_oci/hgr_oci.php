	...
	class HGR_OCI {
		...
		*/
		function display_demos() {
				...
				$return ='<div class="wrap"><h2>'.esc_html( $theme->get( 'Name' ) ).' '.esc_html__( 'OneClick Demo Setup', 'hatch' ).'</h2>';
				
				if( is_array( $this->configFile ) && !is_null( $this->configFile ) ){
					$return .='<p><div class="import_message">'.esc_html__( 'Click on the desired demo to install demo content and theme options setup.', 'hatch' ).'</div><p>';
					$return .='<p><input type="checkbox" name="resetall" id="resetall" value="true"> '.esc_html__( 'Reset WordPress Database? This will erase all content (posts, pages, etc) and install demo content.', 'hatch' ).'<p>';
					...
					foreach( $this->configFile as $demo => $demoContent ) {
						$return .='<div class="demo_container" style="float:left;margin:20px;text-align:center;max-width:315px;background-color:#fff">
							...
							<button style="margin:5px;background-color:#fff;border:1px solid #dedede;cursor:pointer;color:#000;padding:10px;width:143px;">'.esc_html__( 'PREVIEW', 'hatch' ).'</button>
							</a> 
							<button class="bootstrapguru_import" data-install="' . esc_attr( $demo ) . '" data-reset="false" style="margin:5px;background-color:#5194ec;border:0;cursor:pointer;color:#fff;padding:10px;width:143px;">'.esc_html__( 'INSTALL', 'hatch' ).'</button>
							</div>';
					}
				
					$return .='</div>';
				} else {
					$return .='<p><div class="import_message">'.esc_html__( "It looks like the config file for the demos is missing or conatins errors!. Demo install can\'t go futher!", 'hatch' ).'</div><p>';
				}
				$return .='</div>';
			...
			if ( class_exists( 'Essential_Grid_Import' ) ) {
				foreach($essGridFileArray as $essGridFile){
					...
					if( $json_file['response'][code] == '200' && !empty($json_file['body']) ){
						...
						return esc_html__( 'Essential Grids could not be imported.', 'hatch' ).'<br>';
					}
				}
			} else {
				return esc_html__( 'It looks like you don\'t have Essential Grid installed and activated. Grids were not imported!', 'hatch' ).'<br>';
			}
		}
		...
		function importThemeOptions($themeOptionsFile){
			...
			if( $json_file['response']['code'] == '200' && !empty($json_file['body']) ){
				if(update_option( 'redux_options', json_decode($json_file['body'], true), '', 'yes' )){
					echo esc_html__( 'Theme Options saved.', 'hatch' ).'<br>';
				}
			} else{
				echo esc_html__( 'Theme Options could not be imported', 'hatch' ).'<br>';
			}
		}
