		...
		function hgr_install(){
			...
			*/
			if( !$this->hgr_theme_dependency_check() ) {
				$notices[]	=	__("HGR Essentials its only available with <b>".$theme->name."</b> theme. You are not allowed to use this outside <b>".$theme->name."</b> theme.", "hgressentials");
			} else {
				$notices[]	=	__("HGR Essentials its activated now! Thank you for using <b>".$theme->name."</b> Theme.", "hgressentials");
			}
			...
			if( $hgr_query->have_posts() ) {
				...
				*/
				...
				while ( $hgr_query->have_posts() ) {
						...
						if( $i <= $carousel_testimonials_number ) {
							$carousel_content .='<li>
										...
										<div class="cd-author">
											<span class="cd-author-info">'.$hgr_testi_author.',</span>
											<span class="cd-author-info">'.$hgr_testi_position.'</span>
										</div> <!-- cd-author -->
									</li>';
							$i++;
						}
				...
				$output .='</ul> <!-- cd-testimonials -->
						<a href="#0" class="cd-see-all" style="background-color:'.$carousel_bottom_bar_color.'; color:'.$testimonial_text_color.'"></a>
